...
const SportDropdown = ({ activeSport, setActiveSport, categories, sports }) => {
	...
	const [navPosition, setNavPosition] = useState({ top: 0, height: 0 });

	// Đo vị trí và chiều cao của navigation khi component mount
	useEffect(() => {
		const updateNavPosition = () => {
			const navElement = document.querySelector('nav');
			if (navElement) {
				const rect = navElement.getBoundingClientRect();
				const navBottom = rect.bottom + window.scrollY;
				setNavPosition({
					top: navBottom,
					height: rect.height,
				});
	...
	useEffect(() => {
		const handleClickOutside = (event) => {
			// Kiểm tra xem click có phải ở ngoài dropdown và không phải trên sport-item
			...
			if (
				...
				// Kiểm tra xem click có phải ở header không
				headerElement &&
	...
	return (
		<>
			...
			<div
				...
				className="absolute left-12 right-12 top-full mt-1 bg-white rounded-lg shadow-xl border border-gray-200 z-30 flex max-h-[450px] overflow-hidden"
			>
				...
				<div className="w-1/4 border-r border-gray-200 flex flex-col bg-gray-50">
					...
					<div className="bg-gradient-to-r from-blue-600 to-blue-700 py-3 px-4 sticky top-0 z-10">
						<div className="flex items-center text-white">
							<i
								className={`${activeSportIcon} text-xl mr-2`}
							></i>
							...
							<div className="absolute inset-0 bg-gradient-to-r from-blue-900/60 to-transparent flex items-center">
								<div className="p-6 text-white max-w-md">
									...
									<p className="text-sm mb-4 text-blue-100">
										Trang bị đầy đủ với các sản phẩm chất
										lượng cao dành cho môn{' '}
			...
			{/* Overlay chỉ làm mờ phần nội dung, không làm mờ header và nav */}
			<div
				className="fixed inset-0 bg-black/10 z-20"
				style={{ top: navPosition.top }}
